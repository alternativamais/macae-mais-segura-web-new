...
import { UserIpRuleFormDialog } from "./user-ip-rule-form-dialog"
...
import {
  ...
  getRuleValue,
  getUserDisplayName,
  getUserIpMatchTypeLabel,
} from "./utils"
import { useTranslator } from "@/lib/i18n"

interface UserIpRulesTabProps {
  users: User[]
}

export function UserIpRulesTab({ users }: UserIpRulesTabProps) {
  const { hasPermission } = useHasPermission()
  const canRead = hasPermission("listar_restricoes_usuario_ip")
  const canCreate = hasPermission("criar_restricao_usuario_ip")
  const canUpdate = hasPermission("atualizar_restricao_usuario_ip")
  const canDelete = hasPermission("deletar_restricao_usuario_ip")

  const [items, setItems] = useState<AccessUserIpRule[]>([])
  ...
  const [selectedItem, setSelectedItem] = useState<AccessUserIpRule | undefined>()
  const [detailsItem, setDetailsItem] = useState<AccessUserIpRule | null>(null)
  const [isDetailsOpen, setIsDetailsOpen] = useState(false)
  const [itemToDelete, setItemToDelete] = useState<AccessUserIpRule | null>(null)
  const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false)
  const [isDeleting, setIsDeleting] = useState(false)
  const t = useTranslator("access_control.user_ip_rules_tab")
  ...
  const loadItems = useCallback(async () => {
    ...
    setIsLoading(true)
    try {
      const data = await accessControlService.findUserIpRules(
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter((item) =>
      [
        ...
        getUserIpMatchTypeLabel(item.matchType, tAccess),
        getRuleValue(item),
      ]
        .filter(Boolean)
        .join(" ")
  ...
  const handleEdit = (item: AccessUserIpRule) => {
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true)
    try {
      await accessControlService.deleteUserIpRule(itemToDelete.id)
  ...
  return (
    <div className="space-y-4">
            ...
            {paginatedItems.length > 0 ? (
              paginatedItems.map((item) => (
                <TableRow key={item.id}>
                  ...
                  <TableCell>{getUserIpMatchTypeLabel(item.matchType, tAccess)}</TableCell>
      ...
      <UserIpRuleFormDialog
        open={isFormOpen}
        onOpenChange={setIsFormOpen}
        onSuccess={loadItems}
        users={users}
        userIpRule={selectedItem}
      />
        ...
        items={
          detailsItem
            ? [
                ...
                { label: t("details_type"), value: getUserIpMatchTypeLabel(detailsItem.matchType, tAccess) },
