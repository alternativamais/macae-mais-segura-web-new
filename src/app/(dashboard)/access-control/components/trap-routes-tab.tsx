...
import { TrapRouteFormDialog } from "./trap-route-form-dialog"
...
export function TrapRoutesTab() {
  const { hasPermission } = useHasPermission()
  const canRead = hasPermission("listar_rotas_armadilha")
  const canCreate = hasPermission("criar_rota_armadilha")
  const canUpdate = hasPermission("atualizar_rota_armadilha")
  const canDelete = hasPermission("deletar_rota_armadilha")

  const [items, setItems] = useState<AccessTrapRoute[]>([])
  const [isLoading, setIsLoading] = useState(false)
  const [searchTerm, setSearchTerm] = useState("")
  const [page, setPage] = useState(1)
  const [pageSize, setPageSize] = useState(10)
  const [isFormOpen, setIsFormOpen] = useState(false)
  const [selectedItem, setSelectedItem] = useState<AccessTrapRoute | undefined>()
  const [detailsItem, setDetailsItem] = useState<AccessTrapRoute | null>(null)
  const [isDetailsOpen, setIsDetailsOpen] = useState(false)
  const [itemToDelete, setItemToDelete] = useState<AccessTrapRoute | null>(null)
  const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false)
  const [isDeleting, setIsDeleting] = useState(false)
  const t = useTranslator("access_control.trap_routes_tab")
  ...
  const loadItems = useCallback(async () => {
    ...
    setIsLoading(true)
    try {
      const data = await accessControlService.listTrapRoutes()
      setItems(data)
    } catch (error) {
  ...
  const handleEdit = (item: AccessTrapRoute) => {
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true)
    try {
      await accessControlService.deleteTrapRoute(itemToDelete.id)
    ...
    <div className="space-y-4">
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead>{t("col_autoblock")}</TableHead>
            ...
            {paginatedItems.length > 0 ? (
              paginatedItems.map((item) => (
                <TableRow key={item.id}>
                  ...
                  <TableCell>{item.autoBlock ? t("autoblock_yes") : t("autoblock_no")}</TableCell>
                  <TableCell>
      ...
      <RuleDetailsDialog
        ...
        items={
          detailsItem
            ? [
                { label: t("details_path"), value: detailsItem.path },
                { label: t("details_method"), value: detailsItem.method },
                { label: t("details_autoblock"), value: detailsItem.autoBlock ? t("autoblock_yes") : t("autoblock_no") },
