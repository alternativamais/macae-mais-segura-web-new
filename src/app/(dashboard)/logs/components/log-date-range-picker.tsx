...
interface LogDateRangePickerProps {
  dateFrom: string
  dateTo: string
...
function formatRangeLabel(range: DateRange | undefined, placeholder: string, locale: "pt-BR" | "en-US") {
...
export function LogDateRangePicker({
  ...
  return (
    <div className="space-y-2">
      <label className="block text-sm font-medium">{t("fields.period")}</label>
      <div className="flex gap-2">
        <Popover>
          <PopoverTrigger asChild>
            ...
            >
              ...
              {formatRangeLabel(selectedRange, t("placeholders.period"), locale)}
            </Button>
          </PopoverTrigger>
        ...
        {selectedRange?.from ? (
          ...
          >
            <X className="h-4 w-4" />
            <span className="sr-only">{t("actions.clear_period")}</span>
          </Button>
        ) : null}
      </div>
    </div>
