...
export function ChatHeader({
  ...
  const t = useTranslator("chat")

  if (!conversation) {
    return (
      <div className="flex items-center justify-center h-full">
        <p className="text-muted-foreground">{t("no_conversation")}</p>
      </div>
    )
  }
  ...
  const getStatusText = () => {
    if (conversation.type === "group") {
      ...
      switch (primaryUser.status) {
        case "online":
          return t("header.status_online")
        case "away":
          return t("header.status_away")
        case "offline":
          return t("header.status_offline", { 
            date: new Date(primaryUser.lastSeen).toLocaleDateString() 
          })
        default:
          return ""
      }
