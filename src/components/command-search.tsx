...
export function CommandSearch({ open, onOpenChange }: CommandSearchProps) {
  ...
  const t = useTranslator("search")
  const tNav = useTranslator("navigation")

  const searchItems = React.useMemo(
    () => getCommandSearchItems(filterNavigationGroupsByAccess(appNavigation, allowedScreens)),
    [allowedScreens],
  )
        ...
        >
          ...
          <CommandList>
            ...
            {Object.entries(groupedItems).map(([groupKey, items]) => (
              <CommandGroup key={groupKey} heading={tNav(groupKey)}>
                {items.map((item) => {
...
export function SearchTrigger({ onClick }: { onClick: () => void }) {
  ...
  return (
    ...
    >
      ...
      <span className="inline-flex lg:hidden">{t('trigger')}</span>
